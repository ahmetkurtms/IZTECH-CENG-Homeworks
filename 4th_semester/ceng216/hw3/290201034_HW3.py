...
print("#"*25, " PART A ", "#"*25, "\n")
data_points = [(-2, 3), (0, 1), (3, 0), (5, 2)]  # Coordinates of the city
...
b_all = np.array([3, 1, 0, 2]) # Vector b
...
def equation_of_straight_road(t):
    return c_for_straight_road[0] + c_for_straight_road[1] * t # y(t) = c1 + c2*t
...
print("Equation of the straight road: y(t) = ", c_for_straight_road[0], " + ", c_for_straight_road[1], "t \n") #
print("#" * 60)

# Plotting the graph

t1_q1 = np.linspace(-4, 7, 400) 
line1 = equation_of_straight_road(t1_q1) # y(t) = c1 + c2*t
plt.figure(figsize=(10, 6))
plt.plot(t1_q1, line1, color='green',
        label=f'y = {c_for_straight_road[0]:.2f} + {c_for_straight_road[1]:.2f}t')
plt.scatter(t_values, y_values, color='turquoise', label="Data Values") 
...
A_single_curve = np.array([ # Matrix A
    [1, -2, 4],
    [1, 0, 0],
    [1, 3, 9],
    [1, 5, 25]
...
# Squared Error (SE) for each point
...
y1_norm = np.linalg.norm(y1) # ||y1||
q1 = y1 / y1_norm # q1 = y1 / ||y1||

print("y1: ", y1, "\n")
print("q1: ", q1, "\n")

q1_transpose_and_a2 = np.dot(q1.T, A_straight_road[:, 1]) # q1T * a2
y2 = A_straight_road[:, 1] - q1 * q1_transpose_and_a2 # y2 = a2 - q1 * (a2.q1^T)
y2_norm = np.linalg.norm(y2) # ||y2||
q2 = y2 / y2_norm # q2 = y2 / ||y2||

print("y2: ", y2, "\n")
print("q2: ", q2, "\n")

QR = np.column_stack((q1, q2)) # QR factorization
...
q1_transpose_and_a3 = np.dot(q1.T, A_single_curve[:, 2]) # q1T * a3
q2_transpose_and_a3 = np.dot(q2.T, A_single_curve[:, 2]) # q2T * a3
y3 = A_single_curve[:, 2] - q1 * q1_transpose_and_a3 - q2 * q2_transpose_and_a3 # y3 = a3 - q1 * (q1^T * a3) - q2 * (a3 * q2^T)
y3_norm = np.linalg.norm(y3) # ||y3||
q3 = y3 / y3_norm # q3 = y3 / ||y3||
...
y4_norm = np.linalg.norm(y4) # ||y4||
q4 = y4 / y4_norm # q4 = y4 / ||y4||

print("y4: ", y4, "\n")
print("q4: ", q4, "\n")

QR_full = np.column_stack((q1, q2, q3, q4)) # QR factorization
...
values1 = np.linalg.solve(RR_full, vector_d_full) # Solving the equation
